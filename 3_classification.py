...
def classification_LOPO_CV(df, classifier):
    real = []
    prediction = []
    training_times = []
    prediction_times = []
    for id_person in range(1, 49):
        ...
        for i in range(0, len(df_person)):
            ...
            # Łączę rekordy testowe i wybranymi losowo 9 rekordami reszty i dzielę na dane i klasy.
        ...
        for i in range(0, len(df_person)):
            ...
            random_record = df_rest.sample(n=1, random_state=0)
...
df_MinMaxScaler = pd.concat([pd.DataFrame(MinMaxScaler().fit_transform(df_RAW.iloc[:, :-1]), columns=df_RAW.columns[:-1]), df_RAW.iloc[:, -1]], axis=1)
df_StandardScaler = pd.concat([pd.DataFrame(StandardScaler().fit_transform(df_RAW.iloc[:, :-1]), columns=df_RAW.columns[:-1]), df_RAW.iloc[:, -1]], axis=1)
...
data = [df_RAW, df_MinMaxScaler, df_StandardScaler]
data_names = ["Raw", "Norm.", "Scaled"]
...
# Utwórz dokument LaTeX
doc = Document("result")
doc.preamble.append(NoEscape(r'\usepackage{booktabs}'))
doc.preamble.append(NoEscape(r'\usepackage[a4paper,margin=1in]{geometry}'))
...
for j in range(0, len(classifiers)):
    ...
    for i in range(0, len(data)):
        results = classification_LOPO_CV(data[i], classifiers[j])
    ...
    # Dodaj tabelę do pliku LaTeX
    latex_table = df_display.to_latex(index=False, column_format='llrrrl', multicolumn=False)
    doc.append(NoEscape(r'\section*{%s}' % name))
